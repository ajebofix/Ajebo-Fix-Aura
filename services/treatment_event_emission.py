"""Canonical Treatment Plan event adapter for Wave 2.3.

The durable VehicleEvent write remains owned by ``services.event_emission``.
This module registers the Treatment Plan event family with that canonical
emitter and enforces the treatment-specific transition/authority contract
before delegating. It deliberately does not commit.
"""
...
_ADVISOR_ONLY_EVENTS = frozenset(
    {
        "treatment.proposed",
        "treatment.scheduled",
        "treatment.started",
...
_OWNER_ONLY_EVENTS = frozenset({"treatment.authorized"})
_OWNER_OR_ADVISOR_EVENTS = frozenset(
...
def _register_with_canonical_emitter() -> None:
    """Extend the single canonical emitter with the Wave 2.3 event family."""

    canonical_events.TREATMENT_PLAN_EVENT_TYPES = TREATMENT_PLAN_EVENT_TYPES
    canonical_events.CANONICAL_EVENT_TYPES = (
        canonical_events.CANONICAL_EVENT_TYPES | TREATMENT_PLAN_EVENT_TYPES
    )
    ...
    *,
    ...
    if event_type == "treatment.proposed":
        ...
        valid = (
            ...
            and previous_state in _NONTERMINAL_STATES
...
def _validate_authority(
    *,
    ...
    if event_type in _ADVISOR_ONLY_EVENTS and authority not in {
        "advisor",
        "administrator",
    }:
        raise TreatmentEventAuthorityError(
    ...
    if event_type in _OWNER_ONLY_EVENTS and authority != "owner":
        raise TreatmentEventAuthorityError(
            f"{event_type} requires current owner authority"
        )

    if event_type in _OWNER_OR_ADVISOR_EVENTS and authority not in {
        "owner",
        "advisor",
        "administrator",
    }:
...
def emit_treatment_plan_event(
    *,
    ...
    This wrapper performs family-specific state and authority checks while the
    shared canonical emitter retains ownership of persistence, active-ownership
    resolution, payload safety and idempotency semantics.
    """
