...
def _create_identity(*, suffix: int, role: str = "user", own_car: bool = True) -> Identity:
    ...
    user = User(
        name=f"Private Evidence Route User {suffix}",
        email=email,
        phone_number=f"+234833000{suffix:04d}",
        role=role,
        is_active=True,
    ...
    if own_car:
        ...
        db.session.add(
            CarOwnership(
                user_id=user.id,
                car_id=car.id,
                plate_number=f"PR-{suffix:03d}-LA",
                mileage_at_transfer=6000,
                is_active=True,
            )
        )
        car_id = car.id
...
def test_owner_retrieves_private_bytes_through_post_only_grant_flow(app, client):
    ...
    content_response = client.post(
        grant_payload["content_endpoint"],
        data={
            "csrf_token": _csrf(client),
            "grant_token": grant_payload["grant_token"],
        },
    )
...
def test_owner_cannot_call_advisor_deletion_route(app, client):
    ...
    response = client.post(
        f"/evidence/{evidence.id}/delete",
        data={"csrf_token": _csrf(client), "reason_code": "invalid_upload"},
    )
    assert response.status_code == 403
    assert response.get_json() == {"error": "evidence_access_denied"}
