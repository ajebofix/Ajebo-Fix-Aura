...
def _create_user(*, suffix: int, role: str = "user") -> User:
    user = User(
        name=f"Evidence Retrieval User {suffix}",
        email=f"evidence-retrieval-{suffix}@example.com",
        phone_number=f"+234822000{suffix:04d}",
        role=role,
        is_active=True,
...
def _create_owned_car(owner: User, *, suffix: int) -> Car:
    car = Car(
        ...
        model="GLE 450",
        year=2024,
        vin=f"W1NEVIDRETRIEVE{suffix:03d}",
    ...
    db.session.add(
        CarOwnership(
            user_id=owner.id,
            car_id=car.id,
            plate_number=f"RV-{suffix:03d}-LA",
            mileage_at_transfer=10000,
            is_active=True,
        )
    )
    db.session.commit()
...
def test_driver_can_only_retrieve_own_client_evidence(app):
    with app.app_context():
        ...
        db.session.add_all(
            [
                CarDriver(car_id=car.id, user_id=driver_one.id, is_active=True),
                CarDriver(car_id=car.id, user_id=driver_two.id, is_active=True),
            ]
        )
        db.session.commit()
        store = MemoryPrivateStore()
        ...
        assert create_retrieval_grant(
            user_id=driver_one.id,
            evidence_id=own.id,
            secret_key=SECRET,
            grant_seconds=GRANT_SECONDS,
        ).token
...
def test_accepted_or_linked_evidence_cannot_be_immediately_deleted(app):
    with app.app_context():
        ...
        with pytest.raises(EvidenceDeletionConflict, match="Accepted"):
            delete_evidence(
                user_id=advisor.id,
                evidence_id=accepted.id,
                reason_code="operational_correction",
                storage_provider=store,
            )
